...
class ReligiousTP:
    ...
    def read_txt(self):
        ...
        '''
        with open(os.path.join("..","..", "data", "5", self.args['filename']), 'r', encoding="utf-8") as f:
            book = f.read()
    ...
    def remove_things(self, book):
        ...
        ''' Preprocessing function to remove digits, newlines, punctuation (expect '.'). In addition, it checks whether the len of the text exceeds the max for spacy
        Input:
    ...
    def compute_metrics(self, dictionary, corpus, texts, limit=30, start=2, step=3, outpath = ".png"):
        """
        ...
        # Computing LDA model for each n of topics to
        for topic_n in range(start, limit, step):
            model = gensim.models.LdaMulticore(corpus=corpus, # Stream of document vectors or sparse matrix of shape
                                               num_topics=topic_n, # The number of requested latent topics to be extracted from the training corpus.
        ...
        print(f"[INFO] saving model of topic {self.args['metric']} ...")
        ...
        if self.args['metric'] == 'coherence':
            ...
            optimal_topic = info.n_topic.loc[0]
        elif self.args['metric'] == 'perplexity':
    ...
    def LDA(self, corpus, id2word, text_processed):
        ...
        self.lda_model = gensim.models.LdaMulticore(corpus=corpus,    #vectorized corpus - list of lists of tuples
                                       ...
                                       passes=10,        #number of full passes over data (similar to epochs in nn)
                                       iterations=100,   #number of times going over single document (rather than corpus)
